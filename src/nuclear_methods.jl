...
function apply_nuclear_orbital_truncation!(mf, mol_neo, config_sel::ConfigSelection)
    ...
    # Get number of quantum nuclei
    n_quantum_nuc = 0
    if pybuiltin("hasattr")(mol_neo, "nuc_num")
        n_quantum_nuc = mol_neo.nuc_num
    end
    ...
    for i in 0:(n_quantum_nuc-1)  # Python uses 0-based indexing
        nuc_key = "n$i"
        
        if haskey(components, nuc_key)
            mf_nuc = components[nuc_key]
            
            if pybuiltin("hasattr")(mf_nuc, "mo_occ") && pybuiltin("hasattr")(mf_nuc, "mo_coeff")
                mo_occ = collect(mf_nuc.mo_occ)
                mo_coeff = collect(mf_nuc.mo_coeff)
                mo_energy = collect(mf_nuc.mo_energy)
                ...
                if n_orb_total > config_sel.max_nuc_orbs
                    ...
                    mf_nuc.mo_occ = new_mo_occ
                    mf_nuc.mo_coeff = new_mo_coeff
                    mf_nuc.mo_energy = new_mo_energy
